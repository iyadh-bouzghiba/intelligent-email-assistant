...
class _FakeStoreNeverReached:
    """Raises AssertionError if any data method is called.

    Used when the 403 must fire before any store interaction.
...
class TestMultiUserIsolationRoutes(unittest.TestCase):
    ...
    # This route binds to jwt_subject (no account_id param). Cross-account
    # isolation is enforced by _lookup_email_record_by_message_id scoping
    # results to the authenticated account. Monkeypatch to return None (message
    # not visible to owner) → 404.
    ...
    # Same pattern as route 17: lookup scoped to jwt_subject returns None → 404.
