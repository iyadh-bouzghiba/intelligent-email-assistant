...
REDIRECT_URI = "http://127.0.0.1:8888/auth/google/callback"
...
async def google_callback(request: Request, code: str):
    ...
    file_path = os.path.join(CREDENTIALS_DIR, f"{email}.json")
...
async def get_briefing():
    ...
    try:
        ...
        # Task 1: Clean Initialization - FIRST LINE
        all_briefings = []
        seen_emails = set()  # Task 2: Deduplication tracker
        
        # Task 1: Bulletproof Directory Check
        ...
        for filename in credential_files:
            try:
                ...
                account_email = filename.replace(".json", "")
                ...
                # Task 2: Token Verification & Migration Compatibility
                ...
                # Safe Gmail Fetch with proper error handling
                try:
                    ...
                    # Task 1: Console Print for debugging
                    import traceback
                    traceback.print_exc()
                    continue

                # Process each email with full field validation
                for email in emails:
                    try:
                        # Field Validation - use .get() with defaults
                        ...
                        if brain:
                            try:
                                ...
                                log_system(f"⚠️ Brain failed for email, using fallback: {str(e)}")
                        ...
                        # Field validation before append
                        briefing = {
        ...
        # Task 1 & 2: FINAL FAIL-SAFE - ALWAYS RETURN 200 OK
        log_system(f"🚨 CRITICAL FAILURE: {str(e)}")
        log_system("=" * 80)
        log_system("FULL TRACEBACK:")
        import traceback
        traceback.print_exc()  # Task 1: Console Print - MANDATORY
        log_system("=" * 80)
        
        # Task 2: Empty-Feed Fail-Safe - Force 200 OK
        return {
            "account": "Error",
            "briefings": [],
...
if __name__ == "__main__":
    ...
    # Using "api:app" here to match your filename
    ...
    try:
        ...
        if not os.path.exists(CREDENTIALS_DIR):
            os.makedirs(CREDENTIALS_DIR, exist_ok=True)
            log_system("⚠️ Vault directory created. No accounts connected.")
            return {"account": "No Accounts", "briefings": []}

        try:
            credential_files = [f for f in os.listdir(CREDENTIALS_DIR) if f.endswith(".json")]
        except Exception as e:
            log_system(f"❌ Failed to read vault directory: {e}")
            return {"account": "Error", "briefings": []}
        ...
        # Task 1: Deep Exception Handling - Wrap entire loop
        for filename in credential_files:
            try:
                cred_path = os.path.join(CREDENTIALS_DIR, filename)
                account_email = filename.replace(".json", "")
                
                # Task 1: Safe JSON Loading
                log_system(f"🔐 Loading credentials for: {account_email}")
                try:
                    with open(cred_path, 'r') as f:
                        token_data = json.load(f)
                ...
                for email in emails:
                    try:
                        # Task 1: Field Validation - use .get() with defaults
                        subject = email.get('subject', 'No Subject')
                        sender = email.get('sender', 'Unknown Sender')
                        date = email.get('date', datetime.datetime.now(datetime.timezone.utc).isoformat())
                        body = email.get('body', '')
                        ...
                        # Task 1: Field validation before append
                        briefing = {
                            "account": account_email,
                            "subject": subject,
                            "sender": sender,
        ...
        # Task 2: Safe Sorting with Fallback
        def sort_key(b):
            try:
                return parser.parse(b.get('date', ''))
            except Exception:
        ...
        log_system(f"✅ Returning {len(all_briefings)} total briefings (deduplicated)")
        return {"account": "Multi-Account Feed", "briefings": all_briefings[:10]}
        
    except Exception as e:
        # Final safety net - should never reach here
        log_system(f"🚨 CRITICAL: Unexpected error in get_briefing: {str(e)}")
        import traceback
        log_system(f"Traceback: {traceback.format_exc()}")
        return {"account": "Error", "briefings": [], "error": str(e)}
